class Enemy{
  ...
  {
      if(isHit(this.x, this.y, this.enemyImg.width, this.enemyImg.height,
      fighter.x, fighter.y, fighter.fighterImg.width, 
fighter.fighterImg.height))
      {return true;}
    return false;
  }
  ...
  boolean isCollideWithBullet()
  {
    for (int i=0; i<bulletCount; i++)
    {
      if (bullets[i]!=null){
      if(isHit(this.x, this.y, this.enemyImg.width, this.enemyImg.height,
    bullets[i].x, bullets[i].y, bullets[i].bulletImg.width, bullets[i].bulletImg.height))
    {bullets[i]=null;return true;}
      }
    }
    return false;
  }
...
void addSlopeEnemy()
{
  ...
  int h = int(t);
  for (int i = 0; i < 5; ++i) {
    enemys[i] = new Enemy((i+1)*-80, h + i * 50 , 
FlightType.ENEMY);
  }
}
void addDiamondEnemy()
{
  ...
  for (int i = 0; i < 8; ++i) {
    if (i == 0 || i == 7) {
      enemys[i] = new Enemy((x_axis+1)*-80, h, 
FlightType.ENEMY);
      x_axis++;
    }
    else if (i == 1 || i == 5){
      enemys[i] = new Enemy((x_axis+1)*-80, h + 1 * 40, 
FlightType.ENEMY);
      enemys[i+1] = new Enemy((x_axis+1)*-80, h - 1 * 40, 
FlightType.ENEMY);
      i++;
      x_axis++;
      ...
      enemys[i] = new Enemy((x_axis+1)*-80, h + 2 * 40, 
FlightType.ENEMY);
      enemys[i+1] = new Enemy((x_axis+1)*-80, h - 2 * 40, 
FlightType.ENEMY);
      i++;
      x_axis++;
    }
